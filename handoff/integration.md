# Live Map — integration.md

## CURSOR PROMPT
"Read handoff/integration.md and execute all steps in order.
Create files first, then modify. Commit at end. Do not stop unless BLOCKING error."
...
async function fetchPositions(cid: string | null): Promise<LiveBeacon[]> {
  try {
    const r = await apiFetch<{ beacons: LiveBeacon[] }>(withCid("/api/v1/telemetry/positions", cid));
    return r.beacons ?? [];
  } catch { return []; }
}
...
function InferenceCard({ inf, cid, onConfirm, onDismiss }: {
  ...
  if (inf.status === "confirmed") return (
    <div className="rounded-md border border-emerald-200 bg-emerald-50 dark:border-emerald-500/30 dark:bg-emerald-950/30 p-3 flex items-center gap-2">
      ...
      <p className="text-xs font-semibold text-emerald-900 dark:text-emerald-100">Work order auto-logged</p>
    </div>
  );
    ...
    <div className="rounded-md border border-amber-200/80 bg-amber-50/60 dark:border-amber-500/30 dark:bg-amber-950/30 p-3 space-y-2 animate-in slide-in-from-top-1 duration-200">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-bold text-amber-900 dark:text-amber-100">Maintenance inference</span>
        </div>
      ...
      <p className="text-xs text-amber-900 dark:text-amber-100">
        ...
        {inf.pm_overdue_days > 0 && <span className="text-amber-600"> · PM {inf.pm_overdue_days}d overdue</span>}
      </p>
      <div className="flex gap-2">
        <button disabled={busy} onClick={async () => {
          setBusy(true);
          await apiFetch(withCid(`/api/v1/telemetry/inferences/${inf.inference_id}/confirm`, cid), { method: "POST" });
          onConfirm();
        ...
        <button disabled={busy} onClick={async () => {
          setBusy(true);
          await apiFetch(withCid(`/api/v1/telemetry/inferences/${inf.inference_id}/dismiss`, cid), { method: "POST" });
          onDismiss();
...
export type LiveFacilityMapProps = { className?: string; compact?: boolean; pollMs?: number; };

export function LiveFacilityMap({ className = "", compact = false, pollMs = 5000 }: LiveFacilityMapProps) {
  ...
  useEffect(() => {
    ...
    void load();
    const id = setInterval(() => void fetchPositions(cid).then(b => { if (mounted.current) setBeacons(b); }), pollMs);
  ...
  usePulseWs(useCallback((evt: PulseWsEvent) => {
    ...
    if (evt.event_type === "demo_inference_confirmed") setInference(p => p ? { ...p, status: "confirmed" } : null);
    if (evt.event_type === "demo_inference_dismissed") setInference(null);
  }, []), true);
  ...
  return (
    <div className={`w-full space-y-3 ${className}`}>
      <div className={`rounded-md border border-ds-border bg-ds-primary shadow-sm ${compact ? "p-2.5" : "p-3 sm:p-4"}`}>
        <div className="flex items-center justify-between mb-2">
          <p className="text-[10px] font-semibold uppercase tracking-wide text-ds-muted">
            Facility · {zones.length > 0 ? `${zones.length} zone${zones.length !== 1 ? "s" : ""}` : "Zone map"}
          </p>
        ...
        <div className="relative w-full overflow-hidden rounded border border-ds-border bg-slate-950/40" style={{ aspectRatio: "16/9" }}>
          {loading && (
          ...
          {!loading && zones.length === 0 && beacons.length === 0 && (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
              ...
              <p className="text-xs text-ds-muted/60 text-center px-4">No zones yet. Set up zones in Zones &amp; Devices.</p>
            </div>
          )}
          ...
          {zones.map((zone, i) => {
            ...
            return (
              <div key={zone.id} className="absolute border border-slate-400/20 bg-slate-400/5 hover:bg-slate-400/10 transition-colors"
                style={{ left: `${(col / cols) * 100}%`, top: `${(row / rows) * 100}%`, width: `${100 / cols}%`, height: `${100 / rows}%` }}>
          ...
          {beacons.map((b) => {
            ...
            return (
              <div key={b.beacon_id} title={`${b.label}${b.pm_overdue ? " · PM overdue" : ""}`}
                ...
                {b.pm_overdue && <div className="absolute inset-0 -m-1.5 rounded-full bg-red-500/20 animate-ping" />}
      ...
      {inference && (
        <InferenceCard inf={inference} cid={cid}
...
class LiveBeaconOut(BaseModel):
    beacon_id: str; label: str; beacon_type: str
    x_norm: float; y_norm: float; zone_id: Optional[str]
    position_confidence: Optional[float]; pm_overdue: bool = False
...
async def get_live_positions(
    ...
    return PositionsOut(beacons=[
        LiveBeaconOut(
            ...
            pm_overdue=False,  # TODO: join pm_tasks overdue check
...
async def confirm_inference(
    ...
    await db.execute(text(
        "UPDATE maintenance_inferences SET status='confirmed', confirmed_at=:now, confirmed_by=:uid "
        "WHERE id=:id AND company_id=:cid"
    ), {"id": inference_id, "cid": str(user.company_id), "now": datetime.now(timezone.utc), "uid": str(user.id)})
    await db.commit()
    return {"ok": True}
...
async def dismiss_inference(
    ...
    await db.execute(text(
        "UPDATE maintenance_inferences SET status='dismissed' WHERE id=:id AND company_id=:cid"
...
ACTION: add after last app.include_router line
...
ACTION: replace FacilityMap import and usage
...
- [ ] LiveFacilityMap renders — empty state if no beacons
- [ ] Zones from API appear as labelled grid cells on map
- [ ] Beacon dots appear and animate when positions update
- [ ] Inference card appears on maintenance_inference_request WS event
- [ ] Old hardcoded FacilityMap rooms no longer shown
