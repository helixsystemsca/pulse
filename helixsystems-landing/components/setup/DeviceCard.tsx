...
function BleSignalIndicator({ tier }: { tier: BleSignal }) {
  ...
  return (
    <div className="flex items-center gap-1.5" title={`BLE recency: ${tier}`}>
      ...
      <span className="text-[10px] font-semibold uppercase tracking-wide text-pulse-muted">
...
function activityRow(label: string, value: string) {
  return (
    <div className="flex items-center justify-between gap-2 text-xs text-pulse-muted">
      <span className="inline-flex items-center gap-1">
        <Activity className="h-3.5 w-3.5 text-slate-400" aria-hidden />
        {label}
      </span>
      <span className="font-medium tabular-nums text-pulse-navy">{value}</span>
    </div>
  );
}
      ...
      <div className={`${cardBase} ${testRing(testListening, testSuccessFlash, testMatchKind)}`}>
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-pulse-navy">{gateway.name}</h3>
              <p className="mt-0.5 font-mono text-xs text-pulse-muted">{gateway.identifier}</p>
            </div>
          </div>
          <div className="flex flex-col items-end gap-2">
            {statusPill(online, "Online", "Offline")}
            {!online ? <WifiOff className="h-4 w-4 text-slate-400" aria-hidden /> : null}
          </div>
        </div>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2 text-sm text-pulse-muted">
          <span className="text-pulse-navy/80">Zone:</span>
          <span>{zoneLabel ?? "—"}</span>
          {onChangeZone ? (
            <button
              type="button"
              onClick={onChangeZone}
              className="ml-auto text-xs font-semibold text-[#2B4C7E] hover:underline"
            >
              Change zone
            </button>
          ) : null}
        ...
        {onTestDetection ? (
          <button
            ...
            className="mt-3 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-[#2B4C7E] shadow-sm hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60"
          >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h3 className="font-semibold text-pulse-navy">{device.name}</h3>
              {emphasizeUnassigned && !assigned ? (
        ...
        <div className="flex flex-wrap gap-2">
          {onAssign && !disableAssignment ? (
            <button
              ...
              className={
                assigned
                  ? "mt-1 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-[#2B4C7E] shadow-sm hover:bg-slate-50"
                  : "mt-1 rounded-lg bg-[#2B4C7E] px-3 py-1.5 text-xs font-semibold text-white shadow-sm hover:bg-[#234066]"
              }
            >
          ...
          {onTestDetection ? (
            <button
              ...
              onClick={onTestDetection}
              disabled={Boolean(testListening)}
              className="mt-1 rounded-lg border border-dashed border-slate-300 bg-slate-50/80 px-3 py-1.5 text-xs font-semibold text-pulse-navy hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-60"
            >
