...
Set the Vercel project **Root Directory** to `helixsystems-landing` when this folder lives inside a monorepo—or import the repo if this **is** the repo root. The project uses the App Router at `app/` and includes `public/` so Vercel detects **Next.js** (see `vercel.json`).
