...
def upgrade() -> None:
    ...
    if not ah.table_exists(conn, "compliance_rules"):
        op.create_table(
            ...
            sa.Column("tool_id", UUID(as_uuid=False), sa.ForeignKey("tools.id", ondelete="CASCADE"), nullable=False),
            sa.Column("required_sop_id", sa.String(128), nullable=True),
            sa.Column("sop_label", sa.String(255), nullable=True),
            sa.Column("time_limit_hours", sa.Integer(), nullable=False, server_default="24"),
            sa.Column("created_at", sa.DateTime(timezone=True), nullable=False),
        )
        op.create_index("ix_compliance_rules_company_id", "compliance_rules", ["company_id"])
        op.create_index("ix_compliance_rules_tool_id", "compliance_rules", ["tool_id"])
        op.create_unique_constraint("uq_compliance_rules_company_tool", "compliance_rules", ["company_id", "tool_id"])
    ...
    if not ah.table_exists(conn, "compliance_records"):
        op.create_table(
            ...
            sa.Column("sop_label", sa.String(255), nullable=True),
            sa.Column("category", sa.String(32), nullable=False, server_default="procedures"),
        ...
        op.create_index("ix_compliance_records_company_id", "compliance_records", ["company_id"])
        op.create_index("ix_compliance_records_user_id", "compliance_records", ["user_id"])
        op.create_index("ix_compliance_records_tool_id", "compliance_records", ["tool_id"])
