...
def upgrade() -> None:
    ...
    addcol(
        "inventory_items",
        "inv_status",
        sa.Column("inv_status", sa.String(32), nullable=False, server_default="in_stock"),
    )
    ...
    addcol(
        "inventory_items",
        "item_condition",
        sa.Column("item_condition", sa.String(32), nullable=False, server_default="good"),
    )
    addcol(
        "inventory_items",
        "reorder_flag",
        sa.Column("reorder_flag", sa.Boolean(), nullable=False, server_default=sa.text("false")),
    )
    ...
    if ah.column_exists(conn, "inventory_items", "assigned_user_id") and not fk_named(
        conn, "fk_inventory_items_assigned_user_id"
    ):
        op.create_foreign_key(
            "fk_inventory_items_assigned_user_id",
            "inventory_items",
            "users",
            ["assigned_user_id"],
            ["id"],
    ...
    if ah.column_exists(conn, "inventory_items", "linked_tool_id") and not fk_named(
        conn, "fk_inventory_items_linked_tool_id"
    ):
        op.create_foreign_key(
            "fk_inventory_items_linked_tool_id",
            "inventory_items",
            "tools",
            ["linked_tool_id"],
            ["id"],
    ...
    if not ah.table_exists(conn, "inventory_module_settings"):
        ...
        op.create_index(
            "ix_inventory_module_settings_company_id",
            "inventory_module_settings",
            ["company_id"],
            unique=True,
        )
...
def downgrade() -> None:
    ...
    if ah.table_exists(conn, "inventory_module_settings"):
        op.drop_table("inventory_module_settings")
