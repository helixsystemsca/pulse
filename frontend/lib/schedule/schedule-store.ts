...
type ScheduleState = {
  ...
  addShift: (partial: Omit<Shift, "id" | "eventType"> & { id?: string; eventType?: ShiftEventType }) => Shift;
  updateShift: (id: string, patch: Partial<Shift>) => void;
  deleteShift: (id: string) => void;
  replaceShiftId: (oldId: string, newId: string, patch?: Partial<Shift>) => void;
  ...
  persist(
    (set, get) => ({
      ...
      addShift: (partial) => {
        const shift: Shift = {
          ...partial,
          eventType: partial.eventType ?? "work",
          id: partial.id ?? newId("shift"),
        };
      ...
      replaceShiftId: (oldId, newId, patch) =>
        set((s) => ({
          shifts: s.shifts.map((sh) => (sh.id === oldId ? { ...sh, ...(patch ?? {}), id: newId } : sh)),
        })),
      ...
      applyPulseScheduleSnapshot: (workers, zones, newShifts) =>
        set((s) => {
          const keyOf = (sh: Shift) =>
            `${sh.workerId ?? ""}|${sh.date}|${sh.startTime}|${sh.endTime}|${sh.shiftCode ?? ""}`;
          const serverKeys = new Set(newShifts.map(keyOf));
          const pending = s.shifts.filter((sh) => {
            if (sh.autoGenerated || sh.shiftKind === "project_task") return false;
            if (!sh.workerId || sh.eventType !== "work") return false;
            if (isPulseApiShiftId(sh.id)) return false;
            return !serverKeys.has(keyOf(sh));
          });
          return {
            workers,
            zones,
            shifts: [...newShifts, ...pending],
