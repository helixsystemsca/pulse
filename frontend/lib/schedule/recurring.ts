...
/**
 * True when the roster already has any persisted Pulse row for this worker/day (not locally generated).
 * Used to suppress recurring template rows so we never stack a synthetic "day" band on top of a real
 * calendar assignment (e.g. afternoon-only workforce shift, or a project_task block).
 */
function hasManagedPulseShiftForDay(base: Shift[], workerId: string, date: string): boolean {
  return base.some((s) => s.workerId === workerId && s.date === date && !s.autoGenerated);
}
...
function buildRecurringShifts(
  ...
  for (const w of workers) {
    ...
    for (const date of visible) {
      ...
      if (off) continue;
      if (hasManagedPulseShiftForDay(base, w.id, date)) continue;
