/**
 * Tenant vendor directory under `/api/inventory/vendors`.
 * System administrators pass `company_id` as with other inventory APIs.
 */
import { apiFetch } from "@/lib/api";

export type InventoryVendorRow = {
  id: string;
  company_id: string;
  name: string;
...
export type InventoryVendorCreateBody = {
  name: string;
  contact_name?: string | null;
  contact_email?: string | null;
  contact_phone?: string | null;
...
export type InventoryVendorListFilters = Partial<{
  ...
  /** Tri-state: omit from query when undefined; boolean when set */
  active: boolean;
}>;
...
export function buildInventoryVendorsQuery(companyId: string | null, filters: InventoryVendorListFilters): string {
...
export async function fetchInventoryVendors(
  companyId: string | null,
  filters: InventoryVendorListFilters,
): Promise<InventoryVendorRow[]> {
  const q = buildInventoryVendorsQuery(companyId, filters);
  return apiFetch<InventoryVendorRow[]>(`/api/inventory/vendors${q}`);
}

export async function createInventoryVendor(
  companyId: string | null,
  body: InventoryVendorCreateBody,
): Promise<InventoryVendorRow> {
  return apiFetch<InventoryVendorRow>(withCompany(`/api/inventory/vendors`, companyId), {
    method: "POST",
    json: body,
  });
}

export async function patchInventoryVendor(
  companyId: string | null,
  id: string,
  body: InventoryVendorPatchBody,
): Promise<InventoryVendorRow> {
  return apiFetch<InventoryVendorRow>(withCompany(`/api/inventory/vendors/${encodeURIComponent(id)}`, companyId), {
    method: "PATCH",
    json: body,
  });
}

export async function deleteInventoryVendor(companyId: string | null, id: string): Promise<void> {
  await apiFetch<void>(withCompany(`/api/inventory/vendors/${encodeURIComponent(id)}`, companyId), {
    method: "DELETE",
  });
}
