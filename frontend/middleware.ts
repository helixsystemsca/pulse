/**
 * App host rewrite: for Pulse app hostnames, `/` → `/login` so the marketing homepage
 * is not served on the app subdomain. Hosts: `PULSE_APP_HOSTNAMES` or defaults in `pulse-host.ts`
 * (includes `vernon.helixsystems.ca` plus legacy panorama/pulse/ops/pps, and
 * `NEXT_PUBLIC_PULSE_APP_URL` host).
 */
