...
import {
  createInventoryContractor,
  deleteInventoryContractor,
  fetchInventoryContractors,
  patchInventoryContractor,
  type InventoryContractorListFilters,
  type InventoryContractorRow,
} from "@/lib/inventoryContractorsService";
...
const TEXT_COLUMNS: {
  field: keyof InventoryContractorRow;
  filterKey: keyof InventoryContractorListFilters;
  label: string;
}[] = [
  ...
  { field: "item_specialty", filterKey: "item_specialty_contains", label: "Trade / specialty" },
...
function draftToApplied(texts: Record<string, string>, activeSel: "" | "true" | "false"): InventoryContractorListFilters {
  ...
  return out as InventoryContractorListFilters;
}
...
function rowToForm(r: InventoryContractorRow): FormState {
  return {
    name: r.name,
...
export function InventoryContractorsPanel({ apiCompany }: { apiCompany: string | null }) {
  ...
  const [applied, setApplied] = useState<InventoryContractorListFilters>({});
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setErr(message || "Could not load contractors.");
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  function openEdit(r: InventoryContractorRow) {
    setDrawerMode("edit");
    setEditingId(r.id);
    setForm(rowToForm(r));
    setDrawerOpen(true);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-sm font-semibold text-pulse-navy">Contractor directory</p>
          <p className="text-xs text-pulse-muted">
            Same fields as vendors — contacts, account references, trade or specialty, and addresses. Filter any column in
            the grid.
          </p>
        </div>
      ...
      <PulseDrawer
        open={drawerOpen}
        title={drawerMode === "create" ? "Add contractor" : "Edit contractor"}
        subtitle="Stored per organization. Use trade / specialty for the work they perform or supply."
        wide
        placement="center"
        onClose={() => {
          setDrawerOpen(false);
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            ...
            <textarea
              ...
              placeholder="e.g. electrical, concrete, HVAC service…"
            />
          </div>
          <div className="sm:col-span-2">
