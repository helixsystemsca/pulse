...
export function ResetDashboardTourButton({
  ...
  const handleReset = useCallback(
    async (e: React.MouseEvent) => {
      ...
      if (
        !window.confirm(
          `Reset the dashboard walkthrough for ${label}? They will see spotlight steps on Overview again (Ask/Search, then each widget).`
        )
      ) {
        return;
      }
  ...
  return (
    <span className={cn("relative inline-flex", className)}>
      <button
        ...
        title="Reset dashboard walkthrough (spotlight steps on Overview)"
        disabled={busy}
