...
function PoolCard(props: (typeof poolControllers)[number]) {
  ...
  return (
    <div className="flex h-full min-h-0 min-w-0 flex-col rounded-md border border-[color-mix(in_srgb,var(--ds-text-primary)_10%,transparent)] bg-white/95 p-[3px] dark:bg-slate-900/70">
      <p
      ...
      <dl className="mt-0.5 grid min-h-0 flex-1 grid-cols-2 grid-rows-[repeat(2,minmax(0,1fr))] gap-x-0.5 gap-y-1 text-[9px] leading-none">
      ...
      <div className="mt-auto flex shrink-0 min-w-0 gap-0.5 pt-0.5">
        <span
          className={cn(
            "min-w-0 flex-1 truncate rounded px-0.5 py-px text-center text-[7px] font-bold uppercase tracking-wide",
            co2FeederActive
...
/** Three pool chemistry cards — fill the widget body (no extra chrome wrapper). */
export function PoolReadingsOpsWidget() {
  return (
    <div
      className="grid h-full min-h-0 w-full min-w-0 flex-1 grid-cols-3 grid-rows-[minmax(0,1fr)] items-stretch gap-[3px]"
      role="group"
