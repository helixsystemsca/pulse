...
type AppMainProps = {
  children: ReactNode;
  className?: string;
  /** When false, do not reserve left-rail space (kiosk / chrome-less layouts). */
  reserveRail?: boolean;
};

/** Reserves horizontal space for the floating left rail so content stays clear when the rail is collapsed. */
export function AppMain({ children, className = "", reserveRail = true }: AppMainProps) {
  const { authed } = usePulseAuth();
  const gutter = authed && reserveRail ? "lg:pl-64" : "";
  return (
