    ...
    <div className="rounded-md border border-amber-200/80 bg-amber-50/60 dark:border-amber-500/30 dark:bg-amber-950/30 p-3 space-y-2 animate-in slide-in-from-top-1 duration-200">
      ...
      <div className="flex gap-2">
        <button
          ...
          onClick={async () => {
            setBusy(true);
            await apiFetch(withCid(`/api/v1/telemetry/inferences/${inf.inference_id}/confirm`, cid), {
              method: "POST",
            });
            onConfirm();
          }}
        ...
        <button
          ...
          onClick={async () => {
            setBusy(true);
            await apiFetch(withCid(`/api/v1/telemetry/inferences/${inf.inference_id}/dismiss`, cid), {
              method: "POST",
            });
            onDismiss();
          }}
...
export function LiveFacilityMap({ className = "", compact = false, pollMs = 5000 }: LiveFacilityMapProps) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      void fetchPositions(cid).then((b) => {
        if (mounted.current) setBeacons(b);
      });
    }, pollMs);
    return () => {
  ...
  return (
    <div className={`w-full space-y-3 ${className}`}>
      <div
        className={`rounded-md border border-ds-border bg-ds-primary shadow-sm ${
      ...
      >
        <div className="flex items-center justify-between mb-2">
          <p className="text-[10px] font-semibold uppercase tracking-wide text-ds-muted">
            Facility ·{" "}
            {zones.length > 0 ? `${zones.length} zone${zones.length !== 1 ? "s" : ""}` : "Zone map"}
          </p>
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => void load()} className="text-ds-muted hover:text-ds-foreground" type="button">
          ...
          {beacons.map((b) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`absolute left-1/2 top-3.5 -translate-x-1/2 whitespace-nowrap text-[8px] font-bold pointer-events-none ${s.lbl}`}
                >
                  {b.label}
                </span>
              </div>
