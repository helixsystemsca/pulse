...
export function TrainingComplianceDashboard() {
  ...
  useEffect(() => {
    ...
    if (hash.includes("matrix") || hash.includes("training-matrix")) {
      setTab("matrix");
    }
  }, []);
    ...
    <div className="min-w-0 space-y-6">
      <header className="flex flex-col gap-3 border-b border-slate-200/90 pb-4 dark:border-slate-800">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h2 className="text-lg font-bold tracking-tight text-slate-900 dark:text-slate-50">Workforce compliance</h2>
            <p className="mt-1 max-w-3xl text-sm text-slate-600 dark:text-slate-300">
              Operational readiness — training matrix, completion and readiness rates, expirations, and supervisor
